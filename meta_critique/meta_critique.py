...
class MetaCritique:
    def __init__(self, model_type="gpt-4", batch_size=5, api_key=None, api_base=None, seed=None, cache_dir="tmp_cache"):
        ...
        self.batched_openai_engine = OpenAIChat(api_key=cur_config.api_key, api_base=cur_config.api_base, org_id=cur_config.org_id, model=cur_config.model_type, seed=cur_config.seed, temperature=cur_config.temperature, max_tokens=cur_config.max_tokens, top_p=cur_config.top_p, frequency_penalty=cur_config.frequency_penalty, presence_penalty=cur_config.presence_penalty, request_timeout=cur_config.request_timeout)
    ...
    def score(self, input_list):
        ...
        if "reference_answer" in input_list[0]:
            ...
            ref_answers = generate_ref_answer(self.batched_openai_engine, input_list, sys_msg_file="prompts/reference_answer.txt", batch_size=self.batch_size, cache_file=os.path.join(self.cache_dir, "ref_answer.json"))
        ...
        if "reference_critique" in input_list[0]:
            for tmp_data in input_list:
                if isinstance(tmp_data["reference_critique"], dict):
                    ...
                    tmp_data["gpt4_critique"] = {"critique": tmp_data["reference_critique"]}
                del tmp_data["reference_critique"]
        else:
            print("generating reference critique ...")
            ref_critiques = generate_ref_critique(self.batched_openai_engine, input_list, sys_msg_file="prompts/reference_critique.txt", batch_size=self.batch_size, cache_file=os.path.join(self.cache_dir, "ref_critique.json"))
            for idx, ref_critique in enumerate(ref_critiques):
                tmp_data = input_list[idx]
                tmp_data["gpt4_critique"] = {"critique": ref_critique["output"]}

        for tmp_data in input_list:
            tmp_data["hypothesis_critique"] = {"critique": tmp_data["hypothesis_critique"]}
        ...
        if "aius" not in input_list[0]["gpt4_critique"]:
            ...
            ref_aius = extract_aius_for_critique(self.batched_openai_engine, ref_critique_list, sys_msg_file="prompts/extract_aius.txt", batch_size=self.batch_size, cache_file=os.path.join(self.cache_dir, "ref_aius.json"))
            for idx, ref_aiu in enumerate(ref_aius):
                tmp_data = input_list[idx]
                tmp_data["gpt4_critique"]["aius"] = ref_aiu["output"].strip().split("\n")

        print("extracting aius from hypothesis critique ...")
        hyp_aius = extract_aius_for_critique(self.batched_openai_engine, hyp_critique_list, sys_msg_file="prompts/extract_aius.txt", batch_size=self.batch_size, cache_file=os.path.join(self.cache_dir, "hyp_aius.json"))
        for idx, hyp_aiu in enumerate(hyp_aius):
            tmp_data = input_list[idx]
            tmp_data["hypothesis_critique"]["aius"] = hyp_aiu["output"].strip().split("\n")

        print("performing precision task ...")
        precision_outputs = eval_aiu_precision(self.batched_openai_engine, input_list, sys_msg_file="prompts/precision.txt", batch_size=self.batch_size, cache_file=os.path.join(self.cache_dir, "hypothesis_precision.json"))

        print("performing recall task ...")
        recall_outputs = eval_aiu_recall(self.batched_openai_engine, input_list, sys_msg_file="prompts/recall.txt", batch_size=self.batch_size, cache_file=os.path.join(self.cache_dir, "hypothesis_recall.json"))

        precision_score, recall_score, f1_score = meta_score(input_list, precision_outputs, recall_outputs)
        for tmp_data in input_list:
